...
(() => {
  ...
  var SelfEvent = class extends RegisterModule {
    ...
    init(app, treemap2) {
      ...
      mixin(app, methods);
      const selfCtx = { treemap: treemap2, self: this };
      const selfEvents = [...primitiveEvents, "wheel"];
      selfEvents.forEach((evt) => {
        nativeEvents.push(bindPrimitiveEvent(treemap2.render.canvas, selfCtx, evt, event));
      });
      const selfEvt = event.bindWithContext(selfCtx);
